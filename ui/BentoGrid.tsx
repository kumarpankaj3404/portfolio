...
export const BentoGridItem = ({
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText("kumarpankaj3404@gmail.com");
    setcopied(true);
  }
  return (
    <div
      ...
      <div className={`${id === 6 && 'flex justify-center flex-col items-center'} h-full`}>
        <div className="w-full h-full absolute">
          {img && (
            <img
              src={img}
              alt={title ? (typeof title === 'string' ? title : "") : img}
          ...
          <div className="flex gap-1 lg:gap-5 w-fit absolute top-0 -right-3 lg:-right-2">
            <div className="flex flex-col gap-1 md:gap-3 lg:gap-5">
              {['React.js', 'Next.js', 'TypeScript', 'Java'].map((item) => (
                <span key={item} className="py-2 lg:py4 lg:px-3 px-3 text-xs lg:text-bases opacity-50 lg:opacity-100 rounded-lg text-center bg-[#10132E]">
                  {item}
                </span>
              ))}
              <span className="lg:py-4 lg:px-3 py-4 px-3  rounded-lg text-center bg-[#10132E]" />
            </div>
            <div className="flex flex-col gap-1 md:gap-3 lg:gap-5 -mt-2">
              <span className="lg:py-4 lg:px-3 py-4 px-3  rounded-lg text-center bg-[#10132E]" />
              {['AWS', 'Express.js', 'Node.js', 'MongoDB'].map((item) => (
                <span key={item} className="py-2 lg:py4 lg:px-3 px-3 text-xs lg:text-bases opacity-50 lg:opacity-100 rounded-lg text-center bg-[#10132E]">
                  {item}
                </span>
              ))}
